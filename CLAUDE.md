...
- **Player** (`Player.cs`, `Node2D`, kein Physik-Kollisionssystem – Bewegung/Reichweite laufen über einfache Distanzberechnung, um ungetestete Kollisions-Layer/Masken zu vermeiden): WASD-Bewegung (`Input.IsPhysicalKeyPressed`, direkt abgefragte Tasten statt Input-Map-Actions), begrenzt auf den Viewport. Visuell nur ein per `_Draw()` gezeichneter Kreis (Messingfarbe) – keine Sprite-Assets vorhanden.
...
- **Deck-Anbindung:** Beim `_Ready()` baut sich `Player` ein **laufeigenes** `Deck` aus `PlayerCardCollection.DeckCards` (`new Deck(PlayerCardCollection.DeckCards)` – kopiert nur die Zusammensetzung, verändert die Sammlung im Deck-Screen nicht). Bei jedem Level-up (`GrantXp`/`LevelUp`) wird `_runDeck.DrawHand(1)` gezogen und dauerhaft als neue `ActiveAbility` ausgerüstet – die Karten-Ziehung bleibt dadurch sichtbarer Teil des Runs, auch ohne Handkarten-Klicks. Auswahl zwischen mehreren gezogenen Karten (statt automatisch die eine gezogene auszurüsten) ist **noch nicht umgesetzt** (Nutzer nannte das als mögliche Erweiterung).
...
- **XP/Level:** 1 XP pro getötetem Gegner (`EnemyGoblin.TakeDamage` → `Player.GrantXp`), 5 XP pro Level (grober erster Balance-Wert, ungetestet). Bei neuer Fähigkeit zeigt `Arena` die gezogene Karte kurz zentriert (`CardView`, wiederverwendet aus dem Deck-Screen) und blendet sie nach ~2 Sekunden wieder aus – keine Bestätigung nötig, damit das Tempo nicht unterbrochen wird.
- **Gegner** (`EnemyGoblin.cs`): läuft direkt auf die Spielerposition zu (kein Pathfinding), greift bei Kontakt automatisch an (verdeckter Wurf, gleiche Werte wie bisher: Höhlengoblin RK 13, 12 HP, Angriffsbonus +4, 1W6+2). `Arena.cs` spawnt alle 1,5 s einen neuen Gegner an einem zufälligen Punkt am Bildschirmrand – **keine Schwierigkeitssteigerung über die Zeit**, das ist ein bewusst offener erster Wurf.
...
- **HUD** (`Arena.tscn`, `CanvasLayer`): HP-Balken/-Text, Überlebenszeit, "Zurück zum Hub" (erscheint erst bei Niederlage). Kein explizites Sieg-Ziel – der Run läuft, bis der Spieler stirbt (klassische Vampire-Survivors-Struktur).

**Bewusste Vereinfachungen dieser ersten Version** (nicht vergessen, wenn's ans Polishing geht): keine Godot-Physik/Kollisionslayer (nur Distanzchecks, Gegner können sich gegenseitig überlappen), keine Schwierigkeits-/Spawnrate-Steigerung über die Zeit, keine Auswahl zwischen mehreren gezogenen Karten beim Level-up, kein Sprite/Animationen (nur gezeichnete Kreise).
