...
Datengrundlage ist weiterhin `PPRogueLite.Meta.PlayerCardCollection` (`scripts/meta/PlayerCardCollection.cs`) – eine statische Klasse, deren Felder den Prozess über Szenenwechsel hinweg überleben. Sie hält zwei Listen (`DeckCards`, `BenchCards`), befüllt über `CardCatalog.BuildWarriorStartingDeck()` (10 Karten) und `CardCatalog.BuildWarriorBenchCards()` (11 zusätzliche Karten: 2× Hieb, 2× Wuchtschlag, 1× Parade, 1× Finte, 1× Atem holen, plus die vier Modifikatorkarten aus Issue #12/#14 – Kampfrausch/Explosive Heilung/Adrenalin/Giftklinge).
...
**Vier Startkarten** (`scripts/cards/KampfrauschCard.cs`/`ExplosiveHeilungCard.cs`/`AdrenalinCard.cs`/`GiftklingeCard.cs`), alle Test-Balance-Werte:
...
- **Giftklinge** (gekoppelt an `hieb`, Issue #14): 50 % Chance, das getroffene Ziel zu vergiften – erste Karte, die den neuen Gift-Status-Effekt nutzt, siehe eigener Abschnitt unten.

Alle vier starten im Kartenbestand (`CardCatalog.BuildWarriorBenchCards()`), nicht im Startdeck – müssen also erst über den Deck-Screen aktiv ins Deck geholt werden, bevor sie in einem Run gezogen werden können.
...
## Damage-over-Time-Status-Effekte (bereits aufgesetzt, Issue #14)

Erste Umsetzung eines Schaden-über-Zeit-Systems auf `Enemy.cs`, bewusst auf **einen** Effekt beschränkt (Gift) statt gleich Gift/Brand/Blutung gemeinsam zu bauen – passend zum im Issue selbst vorgeschlagenen Vorgehen ("pick one effect type end-to-end before adding burn/bleed variants").

- **Gift-Timer direkt auf `Enemy`** (kein generisches Status-Effekt-System/keine Liste, analog zum bereits bestehenden Slow-Effekt auf `Player`): `_poisonTimer`/`_poisonTickTimer`/`_poisonDamagePerTick`. `Enemy.ApplyPoison(damagePerTick, duration)` setzt/erneuert den Effekt (kein Stacking – eine erneute Anwendung überschreibt Dauer und Schaden pro Tick, gleiches Muster wie `Player.ApplySlow`/`ApplyHaste`).
- **Tick-Mechanismus:** `Enemy.UpdatePoison(delta)` läuft unabhängig vom Bewegungs-Switch (Melee/Ranged/HitAndRun/Boss) jeden Frame in `_Process`, bevor überhaupt geprüft wird, welcher Bewegungstyp vorliegt – ein vergifteter Gegner tickt also unabhängig davon, ob/wie er sich bewegt. Alle `PoisonTickInterval` (1 s) automatischer Schaden über `TakeDamage` (gleicher Pfad wie ein normaler Treffer, inkl. Tod/`GrantXp`/`QueueFree`), eigene giftgrüne Flugtext-Farbe (`PoisonColor`) zur Unterscheidung von normalem Schaden.
- **Visueller Indikator:** solange `_poisonTimer > 0`, zeichnet `Enemy._Draw()` einen dünnen giftgrünen Ring um den Gegner (`DrawArc`, gleiches Prinzip wie der Angriffsreichweite-Ring beim Spieler) – verschwindet automatisch, sobald der Timer abläuft.
- **Ausgelöst über `Giftklinge`** (neue vierte Modifikatorkarte, gekoppelt an `hieb`, siehe Modifikatorkarten-Abschnitt oben): `Player.TriggerAbility` (Fall `"hieb"`) bekommt das getroffene Ziel jetzt als Rückgabewert von `ResolveMeleeAttack` (Signatur geändert von `void` auf `Enemy?`, `null` bei Fehlschlag/keinem Ziel in Reichweite) und reicht es bei einem Treffer an `ApplyCoupledEffect(modifierId, target)` weiter (Parameter `target` neu, Default `null` für Effekte ohne konkretes Trefferziel wie Explosive Heilung). Bei ausgerüsteter Giftklinge: 50 % Chance (`Dice.Roll(100) <= 50`) pro Treffer, das Ziel für 4 Sekunden (4 Ticks) mit 2 Schaden/Tick zu vergiften.
- **Bewusste Vereinfachung:** kein Stacking mehrerer Gift-Anwendungen, kein Immunitäts-/Resistenz-System, nur Gegner können vergiftet werden (nicht der Spieler) – Brand/Blutung sind als eigene Effekttypen noch nicht gebaut, siehe Offene-Punkte-Liste unten.
...
7. **#14 Add damage-over-time status effects** (Gift/Brand/Blutung) – **Gift umgesetzt** (siehe Damage-over-Time-Abschnitt oben: `Enemy.ApplyPoison`, Tick-System, `Giftklinge`-Modifikatorkarte), bewusst als einziger Effekttyp für die erste Version (Issue-Scope: "ein Effekt komplett, bevor Varianten dazukommen"). Brand/Blutung sind noch offen – ob dafür #14 offen bleibt oder eigene Fast-Follow-Issues entstehen (Muster #21/#22 bei #15), ist noch mit dem Nutzer zu klären. Noch nicht vom Nutzer in Godot gegengeprüft.
